...
import magicbook.gtlitecore.client.weidgets.ItemSlotTinyAmountTextWidget;
...
import static magicbook.gtlitecore.api.utils.GTLiteUtility.isInventoryEmpty;

public class MetaTileEntityBudgetCRIBA20 extends MetaTileEntityMultiblockNotifiablePart
    ...
    public MetaTileEntityBudgetCRIBA20(ResourceLocation metaTileEntityId) {
    ...
    protected void initializeInventory() {
        ...
        this.patternSlot = new ItemStackHandler(20) {
        ...
        this.extraItem = new NotifiableItemStackHandler(this, 6, null, false) {
    ...
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new MetaTileEntityBudgetCRIBA20(metaTileEntityId);
    }
    ...
    protected ModularUI createUI(EntityPlayer entityPlayer) {
        ...
        //样板槽位
        WidgetGroup pattern = new WidgetGroup(new Position(7, 20));
        for(int i=0;i<20;i++) {
            pattern.addWidget(new SlotWidget(patternSlot, i, (i%5)*18, (i/5)*18)
                    .setBackgroundTexture(GuiTextures.SLOT, GTLiteGuiTextures.ME_PATTERN_OVERLAY));
        }

        // Item slots
        WidgetGroup slots = new WidgetGroup(new Position(7 + 18 * 5, 20));
        for (int y = 0; y <= 3; y++) {
            for (int x = 0; x <= 3; x++) {
                int index = y * 4 + x;
                slots.addWidget(new ItemSlotTinyAmountTextWidget(patternItems, index, 18 * x, 18 * y,
                    ...
                    public void drawInForeground(int mouseX, int mouseY) {
                        ...
                        if (isMouseOverElement(mouseX, mouseY) && !item.isEmpty()) {
                            ...
                            tooltip.add(TextFormatting.GRAY + I18n.format("gtlitecore.machine.budget_crib.amount_tooltip",
        ...
        WidgetGroup buttons = new WidgetGroup(new Position(7, (int) (18 * 5.25) + 3));

        // Extra item slot
        buttons.addWidget(new SlotWidget(extraItem, 0, 0, 0)
                .setBackgroundTexture(GuiTextures.SLOT).setTooltipText("gtlitecore.machine.budget_crib.extra_item"));
        buttons.addWidget(new SlotWidget(extraItem, 1, 18, 0)
                .setBackgroundTexture(GuiTextures.SLOT).setTooltipText("gtlitecore.machine.budget_crib.extra_item"));
        buttons.addWidget(new SlotWidget(extraItem, 2, 36, 0)
                .setBackgroundTexture(GuiTextures.SLOT).setTooltipText("gtlitecore.machine.budget_crib.extra_item"));
        buttons.addWidget(new SlotWidget(extraItem, 3, 54, 0)
                .setBackgroundTexture(GuiTextures.SLOT).setTooltipText("gtlitecore.machine.budget_crib.extra_item"));
        buttons.addWidget(new SlotWidget(extraItem, 4, 72, 0)
                .setBackgroundTexture(GuiTextures.SLOT).setTooltipText("gtlitecore.machine.budget_crib.extra_item"));
        buttons.addWidget(new SlotWidget(extraItem, 5, 90, 0)
                .setBackgroundTexture(GuiTextures.SLOT).setTooltipText("gtlitecore.machine.budget_crib.extra_item"));
        // Circuit slot
        buttons.addWidget(new GhostCircuitSlotWidget(circuitInventory, 0, 108, 0)
                .setBackgroundTexture(GuiTextures.SLOT, GuiTextures.INT_CIRCUIT_OVERLAY)
                .setConsumer(this::getCircuitSlotTooltip));

        // Blocking toggle
        buttons.addWidget(new ImageCycleButtonWidget(18 * 7, 0, 18, 18, GuiTextures.BUTTON_POWER,
                this::isWorkingEnabled, this::setWorkingEnabled)
                .setTooltipHoverString("gtlitecore.machine.budget_crib.blocking_button"));

        // Return items
        buttons.addWidget(new ClickButtonWidget(18 * 8, 0, 18, 18, "", (clickData) -> returnItems())
                .setButtonTexture(GTLiteGuiTextures.EXPORT).setTooltipText("gtlitecore.machine.budget_crib.return_button"));
    ...
    public void renderMetaTileEntity(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline) {
        ...
        if (isOnline) {
            GTLiteTextures.CRIB_ACTIVE.renderSided(getFrontFacing(), renderState, translation, pipeline);
        } else {
            GTLiteTextures.CRIB_INACTIVE.renderSided(getFrontFacing(), renderState, translation, pipeline);
        }
    }
    ...
    private boolean checkIfEmpty() {
        return isInventoryEmpty(patternItems);
    }
    ...
    public void onDataStickLeftClick(EntityPlayer player, ItemStack dataStick) {
        ...
        dataStick.setTranslatableName("gtlitecore.machine.budget_crib.data_stick_name");
        player.sendStatusMessage(new TextComponentTranslation("gtlitecore.machine.budget_crib.data_stick_use"), true);
    }
    ...
    public void addToolUsages(ItemStack stack, World world, List<String> tooltip, boolean advanced) {
        tooltip.add(I18n.format("gtlitecore.machine.budget_crib.tooltip"));
        tooltip.add(I18n.format("gtlitecore.machine.come_from_gtqt.tooltip"));
